const pluginRE = /^(@mall\/|mall-|@[\w-]+(\.)?[\w-]+\/mall-)cli-plugin-/
const scopeRE = /^@[\w-]+(\.)?[\w-]+\//
const officialRE = /^@mall\//
...
exports.resolvePluginId = id => {
  ...
  if (id === '@mall/cli-service') {
    return id
  }

  if (officialPlugins.includes(id)) {
    return `@mall/cli-plugin-${id}`
  }
  // scoped short
  // e.g. @vue/foo, @bar/foo
  if (id.charAt(0) === '@') {
    ...
    if (scopeMatch) {
      ...
      return `${scope}${scope === '@mall/' ? `` : `mall-`}cli-plugin-${shortId}`
    }
  }
  // default short
  // e.g. foo
