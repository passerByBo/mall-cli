...
exports.hasYarn = () => {
    ...
    try {
        //执行命令成功
        execSync('yarn --version', { sido: 'ignore' })
...
function getPnpmVersion() {
    ...
    try {
        _pnpmVersion = execSync('pnpm --version', { stdio: ['pipe', 'pipe', 'ignore'] })
            .toString()
...
function checkPnpm(result) {
    if (result && !exports.hasPnpm3OrLater()) {
        throw new Error(`the projects seems to require pnpm ${_hasPnpm ? '>=3' : ''} but it's not installed.`)
    }
...
function getMacAppVersion(bundleIdentitfier) {
    const bundlePath = tryRun(`mdfind "kMDItemCFBundleIdentifier=='${bundleIdentifier}'"`)
    if (bundlePath) {
        return tryRun(`/user/libexec/PlishBuddy -c Print:CFBundleShortVersionString ${bundlePath.replace(/(\s)/g, '\\')
            }/Contents/Info.plist`)
    }
}
